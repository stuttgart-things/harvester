# Vault ClusterIssuer Setup for platform

Deploys a Vault-backed ClusterIssuer on the platform cluster using the PKI engine from the infra Vault.

## Prerequisites

- cert-manager running on platform cluster
...
If the platform nodes don't trust the Vault TLS certificate, get the CA and add it to `vault.tf`:
...
```bash
kubectl --kubeconfig=/home/sthings/.kube/platform get clusterissuer vault-pki
```

## Test Certificate

```bash
kubectl --kubeconfig=/home/sthings/.kube/platform apply -f - <<EOF
...
spec:
  ...
  issuerRef:
    name: vault-pki
    kind: ClusterIssuer
  commonName: test.platform.sthings.lab
  dnsNames:
    - test.platform.sthings.lab
EOF
```
