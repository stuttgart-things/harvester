# openSUSE Leap Base Image (Packer + QEMU)

Builds an openSUSE Leap 15.6 QEMU disk image with custom packages and users configured via cloud-init.
...
The output image (`opensuse-leap-base-amd64.img`) will be placed in the `output/` directory. The Harvester upload is skipped by default.
...
```bash
# Custom image name
packer build -var 'image_name=opensuse-leap-custom' .
...
# Custom openSUSE source image
packer build -var 'opensuse_url=https://example.com/custom.qcow2' -var 'opensuse_checksum=sha256:...' .
```
...
|-----------------------|------------------------------------------|--------------------------------------|
| `opensuse_url`        | openSUSE Leap 15.6 NoCloud image URL    | Source cloud image                   |
| `opensuse_checksum`   | SHA256 checksum                          | Checksum for source image            |
| `image_name`          | `opensuse-leap-base`                     | Output image name prefix             |
...
Inputs:
- **packer_dir**: Directory containing the Packer config (default: `harvester/packer/opensuse-leap`)
