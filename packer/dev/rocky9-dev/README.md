# rocky9-dev (dev / playground)

Self-service developer image, **layered on top of the golden `sthings-rocky9`** image
so a build only installs the delta. Onboard yourself by adding your SSH key to
...
The build pulls the golden `sthings-rocky9` base over HTTPS from the MinIO artifact
store (`source_url` in `build.pkrvars.hcl`), published there by the golden build.

> ⚠️ The golden `sthings-rocky9` image must be **built + published at least once**
...
```bash
cd packer/_build
packer build -var-file=../dev/rocky9-dev/build.pkrvars.hcl .
```
